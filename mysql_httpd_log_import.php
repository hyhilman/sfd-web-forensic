...
while (!feof(STDIN)) {
	...
	$logTimestamp = strtotime("{$results['date']} {$results['time']}");
	...
	$timezone = $mysqli->escape_string($results['timezone']);	
	...
	$site = $mysqli->escape_string($results['site']);
...
QQ;
	...
	$logString = "{$remote_host}\t{$ident_user}\t{$auth_user}\t{$timestamp}\t{$timezone}\t{$method}\t{$url}\t{$protocol}\t{$status}\t{$bytes}\t{$referrer}\t{$user_agent}\t{$site}\n";
...
function processLine($line) {
	...
	preg_match('/^(\S+) (\S+) (\S+) \[([^:]+):(\d+:\d+:\d+) ([^\]]+)\] "(\S+) (.+?) (\S+)" (\S+) (\S+) "([^"]+)" "([^"]+)" "(\S+)"$/', $line, $matches);
